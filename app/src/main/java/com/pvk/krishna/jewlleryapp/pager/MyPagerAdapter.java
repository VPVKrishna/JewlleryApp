package com.pvk.krishna.jewlleryapp.pager;
...
import com.pvk.krishna.jewlleryapp.MyCartActivity;
...
ViewPager.OnPageChangeListener {
	...
	public MyPagerAdapter(MyCartActivity context, FragmentManager fm) {
		super(fm);
		this.fm = fm;
	...
	public Fragment getItem(int position) 
	{
		...
		if (position == MyCartActivity.FIRST_PAGE)
			scale = MyCartActivity.BIG_SCALE;     	
		else
		{
			scale = MyCartActivity.SMALL_SCALE;
			IsBlured=true;
	...
	{	
		if (positionOffset >= 0f && positionOffset <= 1f)
		{
			...
			if(cur!=null)
			{
				cur.setScaleBoth(MyCartActivity.BIG_SCALE 
						- MyCartActivity.DIFF_SCALE * positionOffset);
			...
			if(next!=null)
			{
				next.setScaleBoth(MyCartActivity.SMALL_SCALE 
						+ MyCartActivity.DIFF_SCALE * positionOffset);
